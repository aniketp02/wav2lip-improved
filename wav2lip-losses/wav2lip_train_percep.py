...
from models import Wav2Lip, FeatureExtractor
import audio
...
def get_percep_loss(g, gt):
    ...
    for id in range(g.shape[0]):
        gen_features = feature_extractor(g[id])
        real_features = feature_extractor(gt[id]).detach()
        loss_content = criterion_content(gen_features, real_features)
        losses.append(loss_content)
    
    averaged_percep_loss = sum(losses) / len(losses)
    return averaged_percep_loss
...
def train(device, model, train_data_loader, test_data_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_sync_loss, running_l1_loss, running_percep_loss = 0., 0., 0.
...
def eval_model(test_data_loader, global_step, device, model, checkpoint_dir):
    ...
    sync_losses, recon_losses, percep_losses = [], [], []
    step = 0
    while 1:
