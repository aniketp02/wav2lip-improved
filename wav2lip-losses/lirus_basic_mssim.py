...
def save_sample_images(x, g, gt, vanilla_gt, global_step, checkpoint_dir):
    ...
    collage = np.concatenate((refs, inps, gt, g), axis=-2)    
...
def train(device, model, train_data_loader, test_data_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_sync_loss, running_mse_loss, running_msssim_loss = 0., 0., 0.
        ...
        for step, (x, indiv_mels, mel, gt) in prog_bar:
            ...
            vanilla_gt = dummy_img #vanilla_model(indiv_mels, x)

            #print(torch.max(g), torch.max(gt), torch.min(g), torch.min(gt))
            '''
            if hparams.syncnet_wt > 0.:
                ...
                sync_loss = 0.
            '''
            #loss = F.mse_loss(g, gt)
            loss = msssim_loss(g, gt)
            ...
            wandb.log({"Train MS-SSIM Loss": running_mse_loss / (step + 1), "Train Sync Loss": running_sync_loss / (step + 1)})
